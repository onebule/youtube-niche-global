...
export default function AllOpportunityRadar({ locale, onWatch, onCreateIdea, onResearch }: { locale: UiLocale } & OpportunityRadarActions) {
  ...
  return <section className="all-radar-page" aria-label={zh ? '双形态信号总览' : 'Cross-format signal overview'}>
    ...
    <section className="all-radar-feed all-radar-feed-short"><header><div><span className="longform-research-kicker">SHORTS</span><h3>{zh ? 'Shorts 趋势信号' : 'Shorts trend signals'}</h3></div><span>{zh ? '扩散 · 突破 · 供给变化' : 'Spread · breakout · supply change'}</span></header><ShortformOpportunityRadar locale={locale} embedded/></section>
  </section>;
}
