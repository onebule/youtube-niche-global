...
const upstream = 'https://youtube-niche-global-api.vercel.app/api/opportunity-radar';
...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetch(target, {
      headers: { accept: 'application/json', ...(authorization ? { authorization } : {}), ...(forwardedFor ? { 'x-forwarded-for': forwardedFor } : {}) },
      cache: 'no-store',
    });
    ...
    if (!response.ok) {
      ...
      return Response.json(
        { ...payload, error: clientErrorMessage(payload.error, `长视频趋势雷达数据服务暂时不可用（HTTP ${response.status}）。`) },
    ...
    return Response.json({ error: '长视频趋势雷达数据服务暂时不可达，请稍后重试。' }, { status: 502, headers: { 'cache-control': 'no-store' } });
  }
}
