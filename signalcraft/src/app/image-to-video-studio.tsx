...
function modelName(model: VideoModelId) {
  ...
  if (model === 'kling-3') return 'Kling 3.0';
  return 'Auto';
}
      ...
      <div className="studio-layout">
        <form className="studio-form" onSubmit={create}>
          ...
          <div className="studio-controls">
            <label>{text.model}<select name="model" value={model} onChange={event => { const next = event.target.value as VideoModelId; setModel(next); setDuration(current => normalizeVideoDuration(next, current)); if (next === 'minimax-h3' && !['768P', '2K'].includes(resolution)) setResolution('768P'); if (['seedance-2', 'seedance-2-5'].includes(next) && !['480p', '720p', '1080p'].includes(resolution)) setResolution('720p'); }}><option value="auto" disabled>Auto · {locale === 'zh' ? '即将开放' : 'Coming soon'}</option>{models.filter(item => item.id !== 'auto' && item.id !== 'veo-3.1-lite').map(item => <option key={item.id} value={item.id}>{modelName(item.id)}{item.enabled ? '' : ` · ${locale === 'zh' ? '未就绪' : 'Not ready'}`}</option>)}</select></label>
            <label>{text.duration}<select name="duration" value={duration} onChange={event => setDuration(event.target.value)}>{videoDurationOptions(model).map(value => <option key={value}>{value}</option>)}</select></label>
