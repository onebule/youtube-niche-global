...
function ProviderRoutingPanel({ opportunity, locale }: { opportunity: LongformOpportunity; locale: UiLocale }) {
  ...
  return <section className="longform-provider-routing" aria-label={zh ? '长视频模型路由' : 'Long-form model routing'}><div className="longform-provider-routing-head"><div><span className="longform-kicker">P4 PHASE 4.5 · PROVIDER VERIFICATION</span><b>{zh ? '先验证 Provider 合同，再决定是否交给适配器' : 'Verify the provider contract before handing intent to an adapter'}</b><small>{zh ? '已确认内部代理和模型 ID；下游提交、轮询和输出 Schema 仍未验证，未知能力不会被当作已支持。' : 'The internal proxy and model IDs are grounded; downstream submission, polling and output schemas remain unverified.'}</small></div><div className="longform-provider-routing-verdict"><strong>{report.routes.length}</strong><span>{zh ? '个镜头路由' : 'routed specs'}</span><small>{verification ? (zh ? `能力已验证 ${verification.coverage.verified} · 待验证 ${verification.coverage.unknown}` : `${verification.coverage.verified} verified · ${verification.coverage.unknown} unknown`) : (zh ? `未验证模型 ${unverified}` : `${unverified} unverified models`)}</small></div></div>{verification ? <div className="longform-provider-verification-strip"><span>{zh ? '网关：内部代理' : 'Gateway: internal proxy'}</span><span>{zh ? `已配置模型 ${verification.models.length}` : `${verification.models.length} configured models`}</span><span>{zh ? `执行就绪阻塞 ${readiness}` : `${readiness} execution blockers`}</span></div> : null}{decisions.length ? <div className="longform-provider-routing-grid">{decisions.map(decision => <RoutingDecisionCard key={decision.decisionId} decision={decision} locale={locale}/>)}</div> : <div className="longform-provider-routing-empty">{zh ? '当前没有可交给 provider 的生成路线；真实证据、截图、图表和人工拍摄会保持非生成路径。' : 'No provider route is available; real evidence, screenshots, charts and manual capture remain non-generative.'}</div>}</section>;
}
