...
export default function LongformOpportunities({ locale, embedded = false }: { locale: UiLocale; embedded?: boolean }) {
  ...
  return <Container className="longform-page convergence-longform">
    ...
    {data && !error && <section className="longform-scope" aria-label={zh ? '当前评估数据' : 'Current evaluation data'}><p>{zh ? `已读取 ${data.dataScope.longformRows} 条长视频样本 · ${data.opportunities.length} 个候选方向 · 当前筛选显示 ${opportunities.length} 个` : `${data.dataScope.longformRows} long-form samples · ${data.opportunities.length} candidates · ${opportunities.length} visible`}</p>{data.dataScope.latestCapturedAt && <small>{zh ? '最近采集：' : 'Latest capture: '}{data.dataScope.latestCapturedAt}</small>}{lane !== 'ALL' && <button type="button" onClick={() => { setLane('ALL'); updateUrl({ lane: 'ALL' }); }}>{zh ? '清除类型筛选，查看全部方向' : 'Clear lane filter'}</button>}</section>}
    ...
    <nav className="longform-lane-tabs" aria-label={zh ? '机会类型' : 'Opportunity lanes'}>{laneOptions.map(item => <button type="button" key={item.key} className={lane === item.key ? 'active' : ''} onClick={() => { setLane(item.key); setSelectedOpportunityKey(null); updateUrl({ lane: item.key, direction: undefined }); }}>{item.label}{item.key !== 'ALL' && data ? <small>{data.lanes[item.key] || 0}</small> : null}</button>)}<span className="longform-lane-note">{opportunities.length ? (zh ? `当前显示 ${opportunities.length} 个方向 · 按独立频道与样本覆盖排序，不等于进入推荐` : `${opportunities.length} directions · sorted by creator and sample coverage, not entry recommendations`) : (zh ? '当前筛选暂无方向' : 'No directions match this filter')}</span></nav>
    </details>
