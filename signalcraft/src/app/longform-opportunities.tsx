...
function RoutingDecisionCard({ decision, locale }: { decision: ModelRoutingDecision; locale: UiLocale }) {
  const zh = locale === 'zh';
...
function ProviderRoutingPanel({ opportunity, locale }: { opportunity: LongformOpportunity; locale: UiLocale }) {
  ...
  return <section className="longform-provider-routing" aria-label={zh ? '长视频模型路由' : 'Long-form model routing'}><div className="longform-provider-routing-head"><div><span className="longform-kicker">P4 PHASE 4.5B · EXECUTION CONTRACT</span><b>{zh ? '先验证内部代理合同，再决定是否交给适配器' : 'Verify the internal proxy contract before handing intent to an adapter'}</b><small>{zh ? '提交、轮询、输出、错误和模型专属序列化已从后端源码验证；模型访问仍由 Team 权限与启用状态单独控制。' : 'Submission, polling, output, errors and model-specific serialization are grounded in backend source; Team access and enablement remain separate gates.'}</small></div><div className="longform-provider-routing-verdict"><strong>{report.routes.length}</strong><span>{zh ? '个镜头路由' : 'routed specs'}</span><small>{verification ? (zh ? `能力已验证 ${verification.coverage.verified} · 未声明 ${verification.coverage.unknown}` : `${verification.coverage.verified} verified · ${verification.coverage.unknown} unknown`) : (zh ? `未验证模型 ${unverified}` : `${unverified} unverified models`)}</small></div></div>{verification ? <div className="longform-provider-verification-strip"><span>{zh ? '网关：内部代理 · 下游：APIMart' : 'Gateway: internal proxy · Downstream: APIMart'}</span><span>{zh ? `合同已验证 ${verification.models.length} 个模型` : `${verification.models.length} model contracts verified`}</span><span>{zh ? `访问闸门阻塞 ${readiness}` : `${readiness} access gates blocked`}</span></div> : null}{decisions.length ? <div className="longform-provider-routing-grid">{decisions.map(decision => <RoutingDecisionCard key={decision.decisionId} decision={decision} locale={locale}/>)}</div> : <div className="longform-provider-routing-empty">{zh ? '当前没有可交给 provider 的生成路线；真实证据、截图、图表和人工拍摄会保持非生成路径。' : 'No provider route is available; real evidence, screenshots, charts and manual capture remain non-generative.'}</div>}</section>;
}
