...
function ProviderRoutingPanel({ opportunity, locale }: { opportunity: LongformOpportunity; locale: UiLocale }) {
  ...
  return <section className="longform-provider-routing" aria-label={zh ? '长视频模型路由' : 'Long-form model routing'}><div className="longform-provider-routing-head"><div><span className="longform-kicker">P4 PHASE 4 · MODEL ROUTING</span><b>{zh ? '先验证兼容性，再决定交给哪个适配器' : 'Verify compatibility before handing intent to an adapter'}</b><small>{zh ? '路由只消费 P4.3 规格；当前仓库没有可验证的 MiniMax/Seedance provider schema，因此未验证能力不会被当作已支持。' : 'Routing consumes P4.3 specifications only; repository provider schemas are not verified, so unknown capabilities are never treated as supported.'}</small></div><div className="longform-provider-routing-verdict"><strong>{report.routes.length}</strong><span>{zh ? '个镜头路由' : 'routed specs'}</span><small>{zh ? `未验证模型 ${unverified}` : `${unverified} unverified models`}</small></div></div>{decisions.length ? <div className="longform-provider-routing-grid">{decisions.map(decision => <RoutingDecisionCard key={decision.decisionId} decision={decision} locale={locale}/>)}</div> : <div className="longform-provider-routing-empty">{zh ? '当前没有可交给 provider 的生成路线；真实证据、截图、图表和人工拍摄会保持非生成路径。' : 'No provider route is available; real evidence, screenshots, charts and manual capture remain non-generative.'}</div>}</section>;
}
