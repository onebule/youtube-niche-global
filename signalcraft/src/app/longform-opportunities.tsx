...
import { AiProductionFitEmptyState, CreatorProfileFilters, DecisionWorkbench } from './discovery-workbench';
import { fromLongform, isActionableMicroNiche, type ProductionHandoff } from '@/src/lib/product-convergence';
...
import type { GenerationSpecificationReadiness, VisualGenerationSpecificationReport } from '@/src/lib/visual-generation-specification';
import type { CompatibilityState, ModelRoutingDecision, ProviderRoutingReport } from '@/src/lib/provider-routing';
import { buildGenerationSpecV2, createManualGenerationJob, type GenerationJob } from '@/src/lib/video-generation';
...
function DecisionSummary({ opportunity, locale }: { opportunity: LongformOpportunity | null; locale: UiLocale }) {
  ...
  if (!opportunity) return <section className="longform-decision-summary insufficient"><div className="longform-decision-summary-head"><div><span className="longform-kicker">DECISION SUMMARY</span><h2>{zh ? '当前还没有可直接进入的细分赛道。' : 'There is no actionable micro-niche yet.'}</h2></div><span className="longform-recommendation insufficient">{zh ? '继续观察' : 'KEEP WATCHING'}</span></div><p className="longform-summary-empty">{zh ? '已发现的分类信号会继续保留为证据，但在补足具体主题与内容机制前，不会被包装成“现在就该做”的频道方向。' : 'Category-level signals remain as evidence, but they are not presented as a ready-to-make channel direction until topic and mechanism are specific.'}</p></section>;
...
const scriptDevelopmentReadinessLabels: Record<ScriptDevelopmentReadiness, { zh: string; en: string }> = {
  READY_FOR_SCRIPT_WRITING: { zh: '可进入脚本写作', en: 'Ready for script writing' }, READY_WITH_CAUTION: { zh: '谨慎进入脚本写作', en: 'Ready with caution' }, NEEDS_REVISION: { zh: '先修改架构', en: 'Needs revision' }, BLOCKED: { zh: '已阻塞', en: 'Blocked' }, INSUFFICIENT: { zh: '证据不足', en: 'Insufficient' },
};

function ScriptDevelopmentPanel({ opportunity, locale }: { opportunity: LongformOpportunity; locale: UiLocale }) {
  const zh = locale === 'zh';
  const report: ScriptDevelopmentIntelligenceReport | undefined = opportunity.scriptDevelopment;
  if (!report) return null;
  const shown = report.packages.slice(0, 2);
  const readinessLabel = (value: ScriptDevelopmentReadiness) => scriptDevelopmentReadinessLabels[value]?.[zh ? 'zh' : 'en'] || value;
  return <section className="longform-script-development" aria-label={zh ? '长视频脚本架构与场景规划' : 'Long-form script architecture and scene planning'}>
    <div className="longform-script-development-head"><div><span className="longform-kicker">P3 PHASE 4 · SCRIPT ARCHITECTURE</span><b>{zh ? '从创作开发包到脚本架构与场景需求' : 'From creative development to script architecture'}</b><small>{zh ? '只定义段落责任、证据位置、张力与语义场景需求；不生成成稿、逐句旁白、分镜、镜头参数、提示词或 Canvas。' : 'Defines section responsibilities, evidence placement, tension and semantic scene needs; no final prose, storyboard, camera specs, prompts or Canvas.'}</small></div><div className="script-development-verdict"><strong>{report.packages.length}</strong><span>{zh ? '个可写架构' : 'packages'}</span><small>{zh ? `阻塞 ${report.blockedPackages.length}` : `${report.blockedPackages.length} blocked`}</small></div></div>
    {shown.length ? <div className="longform-script-development-list">{shown.map(item => <article key={item.packageId} className={`script-development-card ${item.readiness.toLowerCase()}`}><div className="script-development-card-top"><span>{readinessLabel(item.readiness)}</span><b>{item.confidence}</b></div><h4>{item.scriptArchitecture.structureType} · {item.sections.length} {zh ? '个结构段' : 'sections'}</h4><div className="script-development-grid"><div><small>{zh ? 'Opening' : 'Opening'}</small><strong>{item.scriptArchitecture.openingFunction}</strong></div><div><small>{zh ? '证据计划' : 'Evidence plan'}</small><strong>{item.evidencePlan.requirements.length} · {item.evidencePlan.gaps} {zh ? '待补' : 'gaps'}</strong></div><div><small>{zh ? '承诺交付' : 'Promise delivery'}</small><strong>{item.scriptArchitecture.promiseCoverage.state}</strong></div><div><small>{zh ? '场景需求' : 'Scene needs'}</small><strong>{item.sceneRequirements.length}</strong></div></div><p><strong>{zh ? '结尾责任：' : 'Closing responsibility: '}</strong>{item.scriptArchitecture.closingFunction}</p><details><summary>{zh ? '查看段落职责与证据状态' : 'View section duties and evidence states'}</summary><ol>{item.sections.map(section => <li key={section.sectionId}><b>{section.index}. {section.role}</b><span>{section.objective} · {section.evidenceRequirements[0]?.status || 'UNAVAILABLE'} · {section.visualRequirements[0]?.visualRole}</span></li>)}</ol></details></article>)}</div> : <div className="longform-script-development-empty"><b>{zh ? '当前没有可进入脚本架构的创作开发包' : 'No Creative Development package is ready for script architecture'}</b><span>{zh ? '上游门控、承诺覆盖或原创性检查失败时，系统会保留阻塞结果。' : 'Upstream gates, promise coverage and originality checks remain visible as blocked results.'}</span></div>}
    <div className="longform-script-development-foot"><span>{zh ? `阻塞架构 ${report.blockedPackages.length} 条` : `${report.blockedPackages.length} blocked architectures`}</span><small>{zh ? '来源顺序相似度、转录、视觉理解与验证反馈均按真实可用性标注；所有权重需校准。' : 'Source-sequence similarity, transcript, vision and validation gaps remain explicit; weights require calibration.'}</small></div>
  </section>;
}

const scriptReadinessLabels: Record<ScriptReadiness, { zh: string; en: string }> = {
  READY_FOR_STORYBOARD: { zh: '可进入分镜规划', en: 'Ready for storyboard' }, READY_WITH_CAUTION: { zh: '谨慎进入分镜规划', en: 'Ready with caution' }, NEEDS_REVISION: { zh: '需要修改脚本', en: 'Needs revision' }, BLOCKED: { zh: '已阻塞', en: 'Blocked' }, INSUFFICIENT: { zh: '证据不足', en: 'Insufficient' },
};

function ScriptWritingPanel({ opportunity, locale }: { opportunity: LongformOpportunity; locale: UiLocale }) {
  const zh = locale === 'zh';
  const report: ScriptWritingIntelligenceReport | undefined = opportunity.scriptWriting || opportunity.scriptDraft;
  if (!report) return null;
  const shown: ScriptDraft[] = report.drafts.slice(0, 2);
  const readinessLabel = (value: ScriptReadiness) => scriptReadinessLabels[value]?.[zh ? 'zh' : 'en'] || value;
  return <section className="longform-script-writing" aria-label={zh ? '长视频脚本草稿与旁白智能' : 'Long-form script draft and narration intelligence'}>
    <div className="longform-script-writing-head"><div><span className="longform-kicker">P3 PHASE 5 · SCRIPT WRITING</span><b>{zh ? '证据约束下的可编辑脚本草稿' : 'Evidence-grounded editable script draft'}</b><small>{zh ? '旁白按已验证架构生成，事实主张进入 Claim Registry；缺证据内容保持研究状态，不生成最终录音稿、分镜、镜头、提示词、音频或 Canvas。' : 'Narration follows the validated architecture and every claim enters the Claim Registry; missing evidence stays research-required. No final recording script, storyboard, shots, prompts, audio or Canvas.'}</small></div><div className="script-writing-verdict"><strong>{report.drafts.length}</strong><span>{zh ? '个脚本草稿' : 'drafts'}</span><small>{zh ? `阻塞 ${report.blockedDrafts.length}` : `${report.blockedDrafts.length} blocked`}</small></div></div>
    {shown.length ? <div className="longform-script-writing-list">{shown.map(draft => <article key={draft.scriptId} className={`script-writing-card ${draft.readiness.toLowerCase()}`}><div className="script-writing-card-top"><span>{readinessLabel(draft.readiness)}</span><b>{draft.confidence}</b></div><h4>{draft.scriptVersion} · {draft.sections.length} {zh ? '个段落' : 'sections'}</h4><div className="script-writing-grid"><div><small>{zh ? '承诺交付' : 'Promise'}</small><strong>{draft.promiseDelivery.state}</strong></div><div><small>{zh ? '主张' : 'Claims'}</small><strong>{draft.qa.supportedClaimCount} {zh ? '已支持' : 'supported'}</strong><span>{draft.qa.researchRequiredCount} {zh ? '需研究' : 'research'} · {draft.qa.unsupportedClaimCount} {zh ? '不支持' : 'unsupported'}</span></div><div><small>{zh ? '估算时长' : 'Est. duration'}</small><strong>{draft.pacing.estimatedDurationMinutes === null ? '—' : `${draft.pacing.estimatedDurationMinutes} min`}</strong><span>{draft.pacing.targetWordsPerMinute} WPM · {zh ? '需校准' : 'calibration required'}</span></div><div><small>{zh ? '重复检查' : 'Repetition'}</small><strong>{draft.qa.repetitionCount}</strong><span>{zh ? '段落对' : 'section pairs'}</span></div></div><div className="script-writing-opening"><small>{zh ? 'Opening' : 'Opening'}</small><p>{draft.opening?.narration || (zh ? '暂无开场草稿。' : 'No opening draft yet.')}</p></div><details><summary>{zh ? '查看段落旁白与证据状态' : 'View narration and evidence states'}</summary><ol>{draft.sections.map(section => <li key={section.sectionId}><b>{section.index}. {section.sectionRole}</b><span>{section.narration}</span><small>{section.claimIds.length} claims · {section.evidenceRequirements.length} evidence requirements</small></li>)}</ol></details></article>)}</div> : <div className="longform-script-writing-empty"><b>{zh ? '当前没有可生成的脚本草稿' : 'No Script Draft is ready yet'}</b><span>{zh ? '上游架构、Promise、证据或原创性门控未通过时，系统会保留阻塞状态。' : 'Upstream architecture, promise, evidence or originality gates remain visible as blocked states.'}</span></div>}
    <div className="longform-script-writing-foot"><span>{zh ? `阻塞草稿 ${report.blockedDrafts.length} 条` : `${report.blockedDrafts.length} blocked drafts`}</span><small>{zh ? 'Claims、Evidence、Promise、Pattern、原创性与 pacing 均保留机器可读 provenance；当前不声称最终成稿。' : 'Claims, evidence, promise, pattern, originality and pacing retain machine-readable provenance; this is not a final script.'}</small></div>
  </section>;
}

const storyboardReadinessLabels: Record<StoryboardReadiness, { zh: string; en: string }> = {
  READY_FOR_PRODUCTION_PLANNING: { zh: '可进入制作规划', en: 'Ready for production planning' },
  READY_WITH_CAUTION: { zh: '谨慎进入制作规划', en: 'Ready with caution' },
  NEEDS_REVISION: { zh: '需要修改脚本或场景', en: 'Needs revision' },
  BLOCKED: { zh: '已阻塞', en: 'Blocked' },
  INSUFFICIENT: { zh: '证据不足', en: 'Insufficient' },
};

function StoryboardPlanningPanel({ opportunity, locale }: { opportunity: LongformOpportunity; locale: UiLocale }) {
  const zh = locale === 'zh';
  const report: StoryboardIntelligenceReport | undefined = opportunity.storyboardIntelligence;
  if (!report) return null;
  const storyboards = report.storyboards.slice(0, 2);
  const readinessLabel = (value: StoryboardReadiness) => storyboardReadinessLabels[value]?.[zh ? 'zh' : 'en'] || value;
  return <section className="longform-storyboard-planning" aria-label={zh ? '长视频分镜与镜头规划智能' : 'Long-form storyboard and shot planning intelligence'}>
    <div className="longform-storyboard-planning-head"><div><span className="longform-kicker">P4 PHASE 1 · STORYBOARD PLANNING</span><b>{zh ? '从脚本到可追溯的场景与镜头需求' : 'From Script Draft to traceable scenes and shot requirements'}</b><small>{zh ? '每个场景都回到脚本段落、证据与视觉目的；这里只定义语义需求，不生成图片/视频提示词，也不连接 Canvas。' : 'Every scene traces to a script section, evidence and visual purpose; this defines semantic requirements only—no image/video prompts or Canvas hand-off.'}</small></div><div className="storyboard-planning-verdict"><strong>{report.storyboards.length}</strong><span>{zh ? '个 Storyboard' : 'storyboards'}</span><small>{zh ? `阻塞 ${report.blockedStoryboards.length}` : `${report.blockedStoryboards.length} blocked`}</small></div></div>
    {storyboards.length ? <div className="longform-storyboard-planning-list">{storyboards.map(item => <article key={item.storyboardId} className={`storyboard-planning-card ${item.readiness.toLowerCase()}`}><div className="storyboard-planning-card-top"><span>{readinessLabel(item.readiness)}</span><b>{item.confidence}</b></div><div className="storyboard-planning-stats"><div><small>{zh ? '场景' : 'Scenes'}</small><strong>{item.scenes.length}</strong></div><div><small>{zh ? '镜头需求' : 'Shots'}</small><strong>{item.scenes.reduce((sum, scene) => sum + scene.shots.length, 0)}</strong></div><div><small>{zh ? '资产依赖' : 'Assets'}</small><strong>{item.assetRequirements.length}</strong></div><div><small>{zh ? '证据可视化' : 'Evidence visuals'}</small><strong>{item.evidenceVisualizations.length}</strong></div><div><small>{zh ? '权利复核' : 'Rights review'}</small><strong>{item.productionFeasibility.rightsReviewCount}</strong></div></div><div className="storyboard-planning-scene-list">{item.scenes.slice(0, 4).map(scene => <div key={scene.semanticScene.sceneId}><div><b>{scene.semanticScene.sceneId.split(':').at(-1) || 'Scene'}</b><span>{scene.semanticScene.purpose}</span><em>{scene.semanticScene.suggestedVisualMode}</em></div><p>{scene.semanticScene.visualObjective}</p><small>{scene.shots.length} {zh ? '个镜头需求' : 'shot requirements'} · {scene.feasibility}</small></div>)}</div><div className="storyboard-planning-foot"><span>{zh ? `制作可行性：${item.productionFeasibility.state}` : `Feasibility: ${item.productionFeasibility.state}`}</span><small>{zh ? '时长、拆分和视觉密度阈值均需校准；未知资产与权利状态不会被假设为可用。' : 'Duration, segmentation and visual-density thresholds require calibration; unknown assets and rights are never assumed available.'}</small></div></article>)}</div> : <div className="longform-storyboard-planning-empty"><b>{zh ? '当前没有可规划的 Storyboard' : 'No Storyboard is ready for planning'}</b><span>{zh ? '脚本阻塞、缺少脚本段落或重大视觉矛盾会保留在阻塞状态。' : 'Blocked scripts, missing sections or material visual contradictions remain blocked.'}</span></div>}
  </section>;
}
...
const generationReadinessLabels: Record<GenerationSpecificationReadiness, { zh: string; en: string }> = {
  READY_FOR_MODEL_ADAPTATION: { zh: '可交给模型适配', en: 'Ready for model adaptation' }, READY_WITH_CAUTION: { zh: '谨慎适配', en: 'Ready with caution' }, NEEDS_ASSETS: { zh: '需要补资产', en: 'Needs assets' }, NEEDS_REFERENCE: { zh: '需要参考', en: 'Needs reference' }, NEEDS_RESEARCH: { zh: '需要研究', en: 'Needs research' }, NEEDS_RIGHTS_REVIEW: { zh: '需要权利复核', en: 'Needs rights review' }, NEEDS_REVISION: { zh: '需要修改', en: 'Needs revision' }, BLOCKED: { zh: '已阻塞', en: 'Blocked' }, INSUFFICIENT: { zh: '资料不足', en: 'Insufficient' },
};

function VisualGenerationSpecificationPanel({ opportunity, locale }: { opportunity: LongformOpportunity; locale: UiLocale }) {
  const zh = locale === 'zh';
  const report: VisualGenerationSpecificationReport | undefined = opportunity.visualGenerationSpecifications;
  if (!report) return null;
  const shown = [...report.specifications, ...report.blockedSpecifications].slice(0, 6);
  const counts = { multi: shown.filter(item => item.complexity.state === 'MULTI_STAGE').length, refs: shown.reduce((sum, item) => sum + item.referenceDependencies.filter(ref => ref.required).length, 0), nonGenerative: shown.filter(item => ['MANUAL_PRODUCTION', 'SOURCE_REAL_EVIDENCE', 'SCREEN_CAPTURE', 'CHART_RENDER', 'GRAPHIC_RENDER'].includes(item.generationRoute)).length };
  return <section className="longform-generation-spec" aria-label={zh ? '长视频视觉生成规格' : 'Long-form visual generation specifications'}>
    <div className="longform-generation-spec-head"><div><span className="longform-kicker">P4 PHASE 3 · GENERATION SPEC</span><b>{zh ? '把镜头意图交给未来适配器' : 'Hand semantic shot intent to a future adapter'}</b><small>{zh ? '每个镜头保留 START、CHANGE、END、连续性硬锁、参考依赖与非生成路线；这里不生成模型提示词，也不执行生成。' : 'Each shot keeps START, CHANGE, END, hard continuity locks, reference dependencies and non-generative routes; no model prompt or execution happens here.'}</small></div><div className="generation-spec-verdict"><strong>{report.specifications.length}</strong><span>{zh ? '个可用规格' : 'specifications'}</span><small>{zh ? `阻塞 ${report.blockedSpecifications.length}` : `${report.blockedSpecifications.length} blocked`}</small></div></div>
    <div className="longform-generation-spec-summary"><span>{zh ? '多阶段镜头' : 'Multi-stage shots'} <b>{counts.multi}</b></span><span>{zh ? '必需参考' : 'Required references'} <b>{counts.refs}</b></span><span>{zh ? '非生成路线' : 'Non-generative routes'} <b>{counts.nonGenerative}</b></span><span>{zh ? '阶段版本' : 'Version'} <b>v1</b></span></div>
    {shown.length ? <div className="longform-generation-spec-grid">{shown.map(spec => <article key={spec.specificationId}><div><b>{spec.shotId.split(':').at(-1) || spec.shotId}</b><span>{spec.generationRoute}</span></div><p>{zh ? `准备度：${generationReadinessLabels[spec.readiness]?.zh || spec.readiness}` : `Readiness: ${generationReadinessLabels[spec.readiness]?.en || spec.readiness}`}</p><small>{zh ? `START ${spec.startState.subjects.length || '—'} · CHANGE ${spec.desiredChange.length} · END ${spec.endState ? '有' : '无'} · 锁 ${spec.continuityLocks.length}` : `START ${spec.startState.subjects.length || '—'} · CHANGE ${spec.desiredChange.length} · END ${spec.endState ? 'yes' : 'no'} · locks ${spec.continuityLocks.length}`}</small></article>)}</div> : <div className="longform-generation-spec-empty"><b>{zh ? '暂无可建立的生成规格' : 'No generation specification is available'}</b><span>{zh ? '需要先有分镜与视觉资产包；阻塞状态会保留，不会自动绕过。' : 'Storyboard and visual asset package inputs are required; blocked states are preserved rather than bypassed.'}</span></div>}
  </section>;
}

const compatibilityLabels: Record<CompatibilityState, { zh: string; en: string }> = {
  COMPATIBLE: { zh: '兼容', en: 'Compatible' }, COMPATIBLE_WITH_DEGRADATION: { zh: '兼容但有降级', en: 'Compatible with degradation' }, REQUIRES_ADAPTATION: { zh: '需要适配', en: 'Requires adaptation' }, INCOMPATIBLE: { zh: '不兼容', en: 'Incompatible' }, UNVERIFIED: { zh: '未验证', en: 'Unverified' }, INSUFFICIENT: { zh: '资料不足', en: 'Insufficient' },
};

type ProviderRequest = ProviderRoutingReport['routes'][number]['requests'][number];

function ManualGenerationAction({ decision, request, runtimeReady, locale }: { decision: ModelRoutingDecision; request?: ProviderRequest; runtimeReady: boolean; locale: UiLocale }) {
  const zh = locale === 'zh';
  const [job, setJob] = useState<GenerationJob | null>(null);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const canRun = Boolean(runtimeReady && decision.recommendedModelId && request?.inputAssets?.length && !decision.blockers.length && request.operation === 'IMAGE_TO_VIDEO');
  const run = async () => {
    if (!canRun || busy || !request || !decision.recommendedModelId) return;
    const confirmed = globalThis.confirm(zh ? '只提交 1 个镜头、1 个任务，不自动重试或 fallback。继续吗？' : 'Submit exactly one shot and one job with no retry or fallback?');
    if (!confirmed) return;
    setBusy(true); setError(null);
    try {
      const model = decision.recommendedModelId as Exclude<import('@/src/lib/video-generation').VideoModelId, 'auto'>;
      const duration = `${Math.max(4, Math.round(request.expectedOutput.durationSeconds || 5))}s`;
      const generationSpec = buildGenerationSpecV2({ model, prompt: String(request.payload?.semanticPrompt || '').trim(), referenceMode: 'start-end', startImageAssetId: request.inputAssets[0], duration, aspectRatio: (request.expectedOutput.aspectRatio === '9:16' || request.expectedOutput.aspectRatio === '1:1' ? request.expectedOutput.aspectRatio : '16:9'), resolution: model === 'minimax-h3' ? '768P' : '720p' }, { requestId: `${decision.decisionId}:manual`, idempotencyKey: `${decision.decisionId}:manual` });
      const result = await createManualGenerationJob({ specificationId: decision.specificationId, generationUnitId: decision.generationUnitId, routingDecisionId: decision.decisionId, selectedModelId: model, executionMode: 'MANUAL_SINGLE_JOB', clientActionId: `${decision.decisionId}:manual`, generationSpec });
      setJob(result.job);
    } catch (nextError) {
      setError(nextError instanceof Error ? nextError.message : (zh ? '提交失败。' : 'Submission failed.'));
    } finally { setBusy(false); }
  };
  if (!canRun && !job) return <small className="longform-provider-routing-action-note">{zh ? '需先有可执行的单镜头规格、参考资产和运行时 READY；不会自动提交。' : 'A ready single-shot spec, reference asset and runtime gate are required; nothing submits automatically.'}</small>;
  return <div className="longform-provider-routing-action"><button type="button" onClick={run} disabled={busy}>{busy ? (zh ? '提交中…' : 'Submitting…') : (zh ? '手动生成 1 个镜头' : 'Generate one shot manually')}</button>{job ? <span>{zh ? `任务 ${job.state}` : `Job ${job.state}`}</span> : null}{error ? <small className="longform-provider-routing-blocker">{error}</small> : null}</div>;
}

function RoutingDecisionCard({ decision, locale = 'zh', request, runtimeReady }: { decision: ModelRoutingDecision; locale?: UiLocale; request?: ProviderRequest; runtimeReady: boolean }) {
  const zh = locale === 'zh';
  const recommended = decision.recommendedModelId ? decision.compatibleModels.find(item => item.modelId === decision.recommendedModelId) : null;
  const alternatives = decision.compatibleModels.filter(item => item.modelId !== decision.recommendedModelId).slice(0, 2);
  return <article className="longform-provider-routing-card"><div><b>{decision.generationUnitId.split(':').at(-2) || decision.generationUnitId}</b><span>{decision.mode}</span></div><p>{zh ? '推荐模型：' : 'Recommended: '}<strong>{decision.recommendedModelId || (zh ? '暂无安全推荐' : 'No safe recommendation')}</strong></p>{recommended ? <small>{zh ? `兼容性：${compatibilityLabels[recommended.state]?.zh || recommended.state} · 置信度：${recommended.confidence}` : `Compatibility: ${compatibilityLabels[recommended.state]?.en || recommended.state} · Confidence: ${recommended.confidence}`}</small> : <small>{zh ? '当前能力或可用性尚未验证，不会自动选择。' : 'Capabilities or availability are not verified; AUTO will not select it.'}</small>}<div className="longform-provider-routing-alternatives">{alternatives.map(item => <span key={item.modelId}>{item.modelId} · {compatibilityLabels[item.state]?.[zh ? 'zh' : 'en'] || item.state}</span>)}</div>{decision.userOverride ? <small className="longform-provider-routing-override">{zh ? `手动选择：${decision.userOverride.requestedModelId} · ${decision.userOverride.result}` : `Manual selection: ${decision.userOverride.requestedModelId} · ${decision.userOverride.result}`}</small> : null}{decision.blockers.length ? <small className="longform-provider-routing-blocker">{zh ? `阻塞：${decision.blockers[0].message}` : `Blocked: ${decision.blockers[0].message}`}</small> : null}<ManualGenerationAction decision={decision} request={request} runtimeReady={runtimeReady} locale={locale}/></article>;
}
...
type StructuredDiagnosticUiState =
  | { state: 'IDLE' }
  | { state: 'RUNNING' }
  | { state: 'PASS'; mode?: string; realCallsUsed?: number }
...
function ProductionMaterializationPanel({ record, locale, diagnostic, onDiagnostic }: { record: ProductionMaterialization | null; locale: UiLocale; diagnostic: StructuredDiagnosticUiState; onDiagnostic: () => void }) {
  ...
  return <section className={`longform-production-materialization ${record.state.toLowerCase().replaceAll('_', '-')}`} id="production-materialization" aria-label={zh ? '长视频制作方案' : 'Long-form production plan'}>
    ...
    {canDiagnose ? <div className="longform-structured-diagnostic"><div className="longform-structured-diagnostic-copy"><strong>{zh ? '先验证文本模型契约' : 'Verify the text-model contract first'}</strong><span>{zh ? '仅发送 1 次最小 JSON 文本请求；不创建草案、不生成图片或视频。' : 'One minimal JSON text request only; no draft, image or video is created.'}</span></div><button type="button" onClick={onDiagnostic} disabled={diagnostic.state === 'RUNNING'}>{diagnostic.state === 'RUNNING' ? (zh ? '验证中…' : 'Checking…') : (zh ? '验证结构化输出' : 'Verify structured output')}</button>{diagnostic.state === 'FAIL' ? <small className="longform-structured-diagnostic-error">{diagnostic.message}</small> : null}</div> : null}
    {diagnostic.state === 'PASS' ? <p className="longform-structured-diagnostic-pass">{zh ? `最小结构化诊断已通过（${diagnostic.realCallsUsed || 1} 次文本请求，${diagnostic.mode || 'json_schema'}）。现在可以手动点击“创建制作方案”继续 CreativeBrief；不会自动继续。` : `Minimal structured diagnostic passed (${diagnostic.realCallsUsed || 1} text request, ${diagnostic.mode || 'json_schema'}). Manually click “Create production plan” to continue CreativeBrief; nothing continues automatically.`}</p> : null}
...
export default function LongformOpportunities({ locale, embedded = false }: { locale: UiLocale; embedded?: boolean }) {
  ...
  const [structuredDiagnostic, setStructuredDiagnostic] = useState<StructuredDiagnosticUiState>({ state: 'IDLE' });
  ...
  const load = useCallback(async () => {
    ...
    try {
      const nextData = await fetchLongformOpportunities({ market, language, window, limit: 500, locale }, { signal: controller.signal });
  ...
  }, [language, market, window, locale, zh]);
  useEffect(() => {
    const syncContext = () => {
      ...
      if (urlState.language && ['all', 'en', 'zh', 'ja', 'es', 'pt', 'hi'].includes(urlState.language)) setLanguage(urlState.language);
  ...
  // Lead with independently supported evidence, not a high score from a tiny cluster.
  const classifiedOpportunities = (data?.opportunities || []).filter(item => lane === 'ALL' || item.lanes.includes(lane))
    .sort((a, b) => b.channelCount - a.channelCount || b.sampleSize - a.sampleSize || a.key.localeCompare(b.key));
  const opportunities = classifiedOpportunities.filter(item => isActionableMicroNiche(fromLongform(item)));
  const pendingClassification = classifiedOpportunities.filter(item => !isActionableMicroNiche(fromLongform(item)));
  ...
  const contextOpportunity = useMemo(() => {
    ...
    const direct = data.opportunities.find(item => item.key === researchContext.nicheId && isActionableMicroNiche(fromLongform(item)));
    ...
    const candidates = data.opportunities.filter(item => isActionableMicroNiche(fromLongform(item)) && normalizedText(item.topic) === target &&
  ...
  const runStructuredDiagnostic = async () => {
    if (structuredDiagnostic.state === 'RUNNING' || structuredDiagnostic.state === 'PASS') return;
    setStructuredDiagnostic({ state: 'RUNNING' });
    try {
      const response = await fetch('/api/longform-production', {
        method: 'POST', headers: { accept: 'application/json', 'content-type': 'application/json', ...authHeaders() },
        body: JSON.stringify({ action: 'STRUCTURED_OUTPUT_DIAGNOSTIC' }), cache: 'no-store',
      });
      const payload = await response.json().catch(() => ({}));
      const diagnostic = payload?.structuredOutputDiagnostic;
      if (!response.ok || diagnostic?.state !== 'PASS') throw new Error(typeof payload?.error === 'string' ? payload.error : typeof payload?.code === 'string' ? payload.code : '结构化输出诊断未通过。');
      setStructuredDiagnostic({ state: 'PASS', mode: typeof diagnostic.structuredOutputMode === 'string' ? diagnostic.structuredOutputMode : undefined, realCallsUsed: Number(diagnostic.realCallsUsed) || 1 });
    } catch (reason) {
      setStructuredDiagnostic({ state: 'FAIL', message: reason instanceof Error ? reason.message : '结构化输出诊断未通过。' });
    }
  };
  ...
  return <Container className="longform-page convergence-longform">
    <header className="discovery-header"><span className="longform-kicker">NICHE EVALUATION</span><h1>{zh ? '用视频证据评估一个赛道' : 'Evaluate a niche against video evidence'}</h1><p>{zh ? '先选择研究方向，再看需求、竞争和代表视频。证据不足会明确标注，不等于没有采集数据。' : 'Choose a direction, then inspect demand, competition, and representative videos. Limited evidence is not an empty collection.'}</p></header>
    {data && !error && <section className="longform-scope" aria-label={zh ? '当前评估数据' : 'Current evaluation data'}><p>{zh ? `已读取 ${data.dataScope.longformRows} 条长视频样本 · 已形成 ${opportunities.length} 个可评估细分赛道 · ${pendingClassification.length} 条分类信号仍在识别` : `${data.dataScope.longformRows} long-form samples · ${opportunities.length} actionable micro-niches · ${pendingClassification.length} category signals still classifying`}</p>{data.dataScope.latestCapturedAt && <small>{zh ? '最近采集：' : 'Latest capture: '}{data.dataScope.latestCapturedAt}</small>}{lane !== 'ALL' && <button type="button" onClick={() => { setLane('ALL'); updateUrl({ lane: 'ALL' }); }}>{zh ? '清除类型筛选，查看全部方向' : 'Clear lane filter'}</button>}</section>}
    ...
    {data?.dataAvailability && <section className="longform-scope" aria-label={zh ? '数据来源状态' : 'Data source status'}><div className="longform-scope-copy"><span className="longform-kicker">EVIDENCE STATUS</span><b>{zh ? '哪些数据已进入判断，哪些尚未接入？' : 'What is in the decision, and what is not?'}</b><small>{data.dataAvailability.calibration?.status === 'CALIBRATION_REQUIRED' ? (zh ? '当前阈值待校准：用于同批公开样本的研究排序，不能外推为市场规模或收益。' : 'Threshold calibration is required: rankings apply only to this public cohort.') : (zh ? '阈值已校准。' : 'Thresholds are calibrated.')}</small></div><div className="longform-scope-facts">{data.dataAvailability.sources.map(source => <span key={source.id}><small>{source.id}</small><b>{source.state}</b><small>{source.dataClass}</small></span>)}</div></section>}
    ...
    <nav className="longform-lane-tabs" aria-label={zh ? '机会类型' : 'Opportunity lanes'}>{laneOptions.map(item => <button type="button" key={item.key} className={lane === item.key ? 'active' : ''} onClick={() => { setLane(item.key); setSelectedOpportunityKey(null); updateUrl({ lane: item.key, direction: undefined }); }}>{item.label}{item.key !== 'ALL' && data ? <small>{data.lanes[item.key] || 0}</small> : null}</button>)}<span className="longform-lane-note">{opportunities.length ? (zh ? `当前显示 ${opportunities.length} 个可评估细分赛道` : `${opportunities.length} actionable micro-niches`) : (zh ? '当前筛选尚未形成可评估的细分赛道' : 'No actionable micro-niche in this filter')}</span></nav>
    </details>
    {error ? <div className="longform-state error"><b>{zh ? '暂时无法读取长视频数据' : 'Long-form data is unavailable'}</b><p>{error}</p><button type="button" onClick={() => void load()}>{zh ? '重试' : 'Try again'}</button></div> : loading && !data ? <div className="longform-state"><b>{zh ? '正在整理公开长视频样本…' : 'Preparing public long-form samples…'}</b></div> : opportunities.length ? <>
      ...
      <section className="longform-research-workspace">
        <aside className="longform-direction-index" aria-label={zh ? '选择研究赛道' : 'Choose a niche to research'}>
          <div><span className="longform-kicker">MICRO NICHES</span><b>{zh ? '选择一个可评估的细分赛道' : 'Choose an actionable micro-niche'}</b><small>{zh ? '一级分类不会进入这份列表；每一项都需要具体主题与可复核的内容机制。' : 'Categories do not enter this list. Every item needs a specific topic and a reviewable mechanism.'}</small></div>
          <div className="longform-direction-list">{opportunities.map((item, index) => { const selected = item.key === selectedOpportunity?.key; const recommendation = recommendationFor(item, locale); const microNiche = fromLongform(item).subNiche || `${item.mechanism} · ${item.productionType}`; return <button type="button" key={item.key} className={selected ? 'active' : ''} onClick={() => { setSelectedOpportunityKey(item.key); updateUrl({ direction: item.key }); }} aria-current={selected ? 'true' : undefined}><span><b>{String(index + 1).padStart(2, '0')}</b><strong>{microNiche}</strong><small>{zh ? `来源分类：${item.topic}` : `Source category: ${item.topic}`}</small></span><em className={recommendation.key}>{recommendation.label}</em></button>; })}</div>
        </aside>
        <div className="longform-research-main" id="research-decision"><div className="longform-workspace-heading"><span className="longform-kicker">SINGLE-NICHE DECISION</span><b>{zh ? '当前研究对象' : 'Current research subject'}</b><small>{selectedOpportunity ? fromLongform(selectedOpportunity).subNiche : (zh ? '尚未选择可评估的细分赛道' : 'No actionable micro-niche selected')}</small></div>{materializationError ? <div className="longform-production-error" role="alert">{materializationError}</div> : null}{selectedOpportunity ? <OpportunityCard opportunity={selectedOpportunity} locale={locale} materialization={materialization} onCreateProduction={createProductionPlan} creating={materializing} diagnostic={structuredDiagnostic} onDiagnostic={() => void runStructuredDiagnostic()}/> : <>{!(researchContext?.discovery?.unit && isActionableMicroNiche(researchContext.discovery.unit)) && <AiProductionFitEmptyState locale={locale}/>}<div className="longform-state">{researchContext?.discovery?.unit && isActionableMicroNiche(researchContext.discovery.unit) && <DecisionWorkbench unit={researchContext.discovery.unit} locale={locale} bridgeNote={zh ? '雷达证据已带入，但缺少同一具体方向的制作评估证据。' : 'Radar evidence carried over; matching production evaluation evidence is missing.'}/>}<b>{zh ? '所选雷达方向尚未匹配到长视频评估证据' : 'The selected Radar direction has no matching evaluation evidence'}</b><p>{zh ? '不会自动切换到其他赛道。可从左侧明确选择方向，或返回雷达继续观察。' : 'No unrelated niche is substituted. Select a direction explicitly or return to Radar.'}</p><button type="button" onClick={returnToRadar}>{zh ? '返回趋势雷达' : 'Back to Radar'}</button></div></>}</div>
      </section>
    </> : <><AiProductionFitEmptyState locale={locale}/><div className="longform-state"><b>{zh ? '当前还没有可直接进入的细分赛道' : 'No actionable micro-niche yet'}</b><p>{zh ? `已发现 ${pendingClassification.length} 条分类信号，但它们尚未同时具备具体主题与可复核内容机制。它们会保留为市场证据，不会被当成推荐赛道。` : `${pendingClassification.length} category signals were found, but none yet combines a specific topic with a reviewable content mechanism. They remain market evidence, not recommendations.`}</p>{pendingClassification.length ? <details className="longform-pending-classification"><summary>{zh ? '查看仍在识别的分类信号' : 'View signals still being classified'}</summary><div>{pendingClassification.slice(0, 12).map(item => <p key={item.key}><b>{item.topic}</b> · {item.mechanism} · {item.sampleSize} {zh ? '条样本' : 'samples'}</p>)}</div></details> : null}<button type="button" onClick={() => openRelatedRadar()}>{zh ? '返回趋势雷达继续观察' : 'Return to Trend Radar'}</button></div></>}
