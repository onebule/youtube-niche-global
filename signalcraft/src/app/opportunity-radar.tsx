...
function RadarCard({ event, locale }: { event: OpportunityRadarEvent; locale: UiLocale }) {
  const zh = locale === 'zh';
...
export default function OpportunityRadar({ locale }: { locale: UiLocale }) {
  ...
  const laneOptions = useMemo(() => [{ key: 'ALL', zh: '全部事件', en: 'All events' }, { key: 'EMERGING', zh: '形成中', en: 'Emerging' }, { key: 'SMALL_CREATOR', zh: '中小频道突破', en: 'Small creator' }, ...(data?.lanes.SATURATION ? [{ key: 'SATURATION', zh: '饱和预警', en: 'Saturation' }] : [])], [data]);
  const events = (data?.events || []).filter(event => lane === 'ALL' || (lane === 'SMALL_CREATOR' ? event.eventType === 'SMALL_CREATOR_BREAKOUT' : lane === 'SATURATION' ? event.lifecycle === 'SATURATING' : event.lifecycle === lane));
