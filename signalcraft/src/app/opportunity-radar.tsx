...
import { beginnerAccessForRadar, changeSignalStatusForRadar, competitionForRadar } from '@/src/lib/opportunity-presentation';
...
function RadarCard({ event, locale, onWatch, onCreateIdea, onResearch, onSelect, focused }: { event: OpportunityRadarEvent; locale: UiLocale; onSelect: (event: OpportunityRadarEvent) => void; focused?: boolean } & OpportunityRadarActions) {
  ...
  const decision = changeSignalStatusForRadar(event);
  ...
  return <article className={`radar-v2-card${focused ? ' focused' : ''}`}>
    ...
    <section className="radar-v2-decision-brief"><div><small>{zh ? '变化信号' : 'CHANGE SIGNAL'}</small><b>{event.facts[0] || (zh ? '公开快照中出现了可观察的跨频道变化。' : 'A measurable cross-channel change appeared in public snapshots.')}</b></div><span className={`confidence-${event.confidence.toLowerCase()}`}>{zh ? `${confidence.zh}置信度 · ${event.sampleVideoCount} 条样本` : `${confidence.en} confidence · ${event.sampleVideoCount} samples`}</span></section>
  ...
  useEffect(() => {
    const syncRadarContext = () => {
      ...
      if (shouldRestore && restoreContext?.returnState) {
        ...
        const queryWindow = urlState.window;
