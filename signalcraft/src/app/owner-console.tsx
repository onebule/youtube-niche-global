...
const collectionStatus = (status?: string | null) => {
  if (status === 'completed') return { label: '已完成', tone: 'success' };
  if (status === 'running') return { label: '运行中', tone: 'warning' };
  if (status === 'failed') return { label: '失败', tone: 'warning' };
  return { label: '未运行', tone: 'warning' };
};
...
function TeamAccessDialog({
  ...
  return <div className="team-access-backdrop" role="presentation">
    <section ref={dialogRef} className="team-access-dialog" role="dialog" aria-modal="true" aria-labelledby="team-access-title" tabIndex={-1} onKeyDown={keepFocusInDialog}>
      ...
      {confirmingRevoke ? <p className="team-access-warning">撤销后，该账号恢复 Free 权限（登录后仅可查看前 10 条榜单）；以后可再次开通。</p> : <><p className="team-access-copy">Pro 对应定价页的研究工作流和完整排行榜；Team 在此基础上开放 AI 图生视频、团队成员与共享积分。两种套餐都不会授予站点管理或主人权限。</p><label className="team-access-duration" htmlFor="team-access-plan">开通套餐<select id="team-access-plan" name="team-access-plan" value={plan} onChange={event => setPlan(event.target.value as AccountPlan)} disabled={saving}><option value="pro">Pro · 研究工作流 + 完整排行榜</option><option value="team">Team · Pro 全部能力 + AI 图生视频</option></select></label><label className="team-access-duration" htmlFor="team-access-duration">权限有效期<select id="team-access-duration" name="team-access-duration" value={duration} onChange={event => setDuration(event.target.value as TeamAccessDuration)} disabled={saving}><option value="7d">7 天</option><option value="30d">30 天</option><option value="permanent">长期有效</option></select></label>{isActive && <p className="team-access-current">当前状态：{teamAccessLabel(user)}</p>}</>}
...
export default function OwnerConsole({ account, onSignIn }: { account: AccountSession | null; onSignIn: () => void }) {
  ...
  if (error || !overview) return <main className="page owner-console"><section className="owner-gate"><span className="eyebrow">SERVICE UNAVAILABLE</span><h1>管理服务暂时无法读取。</h1><p>{error || '请稍后刷新重试。'}</p><button className="primary" onClick={refresh}>重新读取管理状态</button><p className="owner-hint">这不会影响已采集的公开数据、注册账号或用户查询额度；它只表示本次管理概览请求没有完成。</p></section></main>;

  const latest = overview.collection.latestRun;
  const latestStatus = collectionStatus(latest?.status);
  return <main className="page owner-console">
    <section className="owner-header"><div><span className="eyebrow">SITE ADMINISTRATION</span><h1>站点管理台</h1><p>当前主人：{overview.owner.email}。这里显示采集数据、历史快照、账号目录和额度服务的运行状态；密钥始终只保留在服务器环境变量中。</p></div><button onClick={refresh}>刷新状态</button></section>
