...
export default function RankingDataScope({ scope, locale }: { scope: PublicRankingScope; locale: UiLocale }) {
  ...
  return <section className="ranking-data-scope" aria-label={copy.scopeLabel} aria-live="polite">
    ...
    <ul><li>{interpolate(copy.scopeMarkets, { count: scope.marketCount })}</li>{scope.source === 'stored-corpus' && <li>{interpolate(copy.scopeCollectionWindow, { days: scope.collectionLookbackDays })}</li>}<li>{interpolate(copy.scopePublishedWindow, { days: scope.publishedWindowDays })}</li><li>{capturedAt ? <time dateTime={scope.latestCapturedAt!}>{interpolate(copy.scopeUpdatedAt, { time: capturedAt })}</time> : copy.scopeLiveNow}</li></ul>
  </section>;
}
