...
function StudioNav({path,locale}:{path:string;locale:UiLocale}){const copy=languageCopy[locale];const items=[['/app',locale==='zh'?'概览':'Overview'],['/app/image-to-video',locale==='zh'?'AI 图生视频':'AI image-to-video'],['/app/canvas',locale==='zh'?'无限画布':'Infinite canvas'],['/app/doctor',copy.nav[3]],['/app/library/channels',locale==='zh'?'竞品频道':'Competitor channels'],['/app/library/videos',locale==='zh'?'收藏研究':'Saved research'],['/app/thumbnails',locale==='zh'?'缩略图研究':'Thumbnail lab'],['/app/cases',locale==='zh'?'爆款拆解':'Viral breakdown'],['/app/research',copy.studioNav[1]],['/app/watchlists',locale==='zh'?'视频警报':'Video alerts'],['/app/benchmarks',locale==='zh'?'竞品对标':'Benchmarking'],['/app/ideas',locale==='zh'?'选题':'Ideas'],['/app/prompts',locale==='zh'?'提示词':'Prompts'],['/app/settings',locale==='zh'?'配置':'Settings']];return <aside className="studio-nav"><p>{locale==='zh'?'工作室':'STUDIO'}</p>{items.map(([href,label])=><button key={href} className={cn(path===href&&'active')} onClick={()=>navigate(href)}>{label}</button>)}</aside>}
...
function VideoCard({video,state,setState,onDetail,locale}:{video:Video;state:Persisted;setState:React.Dispatch<React.SetStateAction<Persisted>>;onDetail:(v:Video)=>void;locale:UiLocale}){
  ...
  return <article className="video-card">
    ...
    <div className="card-body"><div className="eyebrow">{video.topic} · {video.language} / {video.region}</div><button className="video-title" onClick={watch}><span>{video.title}</span>{titleZh&&<small className="title-translation">中文：{titleZh}</small>}</button><p className="channel-line">{channel.title} · {num.format(channel.subscribers)} 订阅</p>
