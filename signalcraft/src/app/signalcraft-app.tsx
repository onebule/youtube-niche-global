...
function StudioNav({path,locale}:{path:string;locale:UiLocale}){const copy=languageCopy[locale];const primary=copy.primaryNav;const items=[['/app',locale==='zh'?'概览':'Overview'],['/app/research',primary.research],['/app/library/channels',locale==='zh'?'竞品频道':'Competitor channels'],['/app/library/videos',locale==='zh'?'研究资料':'Research library'],['/app/thumbnails',locale==='zh'?'缩略图研究':'Thumbnail research'],['/app/benchmarks',locale==='zh'?'竞品对标':'Benchmarking'],['/app/doctor',primary.doctor],['/app/watchlists',primary.monitor],['/app/cases',locale==='zh'?'视频拆解':'Video breakdown'],['/app/ideas',locale==='zh'?'选题':'Ideas'],['/app/canvas',locale==='zh'?'画布':'Canvas'],['/app/image-to-video',locale==='zh'?'图生视频':'Image-to-video'],['/app/prompts',locale==='zh'?'提示词与版本':'Prompts & versions'],['/app/settings',locale==='zh'?'配置':'Settings']];return <aside className="studio-nav"><p>{locale==='zh'?'工作室':'STUDIO'}</p>{items.map(([href,label])=><button key={href} className={cn(path===href&&'active')} onClick={()=>navigate(href)}>{label}</button>)}</aside>}
