...
function RankingBoard({longRows,shortRows,selectedFormat,filters,onDetail,locale,loadedCount,canLoadMore,loadingMore,onLoadMore,emptyMessage}:{longRows:Video[];shortRows:Video[];selectedFormat:'all'|'short'|'long';filters:ReturnType<typeof parseFilters>;onDetail:(v:Video)=>void;locale:UiLocale;loadedCount:number;canLoadMore:boolean;loadingMore:boolean;onLoadMore:()=>void;emptyMessage:string|null}){
  ...
  const renderVideos=()=>videos.length?videos.map((video,index)=>{const score=scoreFor(video),channel=channelFor(video),growth=getRecordedGrowth(video);return <a className="ranking-row" key={video.id} {...externalVideoProps(video.sourceUrl!)} title={locale==='zh'?'在新标签页打开此视频':'Open this video in a new tab'}><span className={cn('ranking-position',index<3&&'top')}>{String(index+1).padStart(2,'0')}</span><span className="ranking-thumb">{video.thumbnail&&<img src={video.thumbnail} alt="" width={160} height={90} loading="lazy" decoding="async"/>}<em>{video.format==='short'?(locale==='zh'?'短':'Short'):(locale==='zh'?'长':'Long')}</em></span><span className="ranking-title"><b>{video.title}</b><small>{channel.title} · {localizedTopic(video.topic,locale)} · {formatCompactNumber(channel.subscribers,locale)} {locale==='zh'?'订阅':'subs'}</small></span><span className="ranking-stat"><b>{formatCompactNumber(video.snapshots.at(-1)!.views,locale)}</b><small>{copy.views}</small></span><span className="ranking-stat velocity"><b>{score.viewsPerSubscriber}×</b><small>{copy.viewsPerSubscriber}</small></span><span className="ranking-stat growth"><b>{growth?`+${formatCompactNumber(growth.views,locale)}`:'—'}</b><small>{growth?interpolate(copy.growthOverHours,{hours:Math.round(growth.hours)}):copy.growthUnavailable}</small></span><span className="ranking-open">{copy.research}</span></a>}):<div className="ranking-empty">{emptyMessage||copy.noVideos}</div>;
  const renderCards=()=>videos.length?<div className="ranking-card-grid">{videos.map((video,index)=>{const channel=channelFor(video),growth=getRecordedGrowth(video);return <a className="ranking-video-card" key={video.id} {...externalVideoProps(video.sourceUrl!)} title={locale==='zh'?'在新标签页打开此视频':'Open this video in a new tab'}><span className="ranking-card-image">{video.thumbnail&&<img src={video.thumbnail} alt="" width={480} height={270} loading={index<3?'eager':'lazy'} fetchPriority={index<3?'high':'auto'} decoding="async"/>}<b>#{index+1}</b><em>{video.format==='short'?(locale==='zh'?'短':'Short'):(locale==='zh'?'长':'Long')}</em></span><span><strong>{video.title}</strong><small>{channel.title} · {localizedTopic(video.topic,locale)}</small><b>{formatCompactNumber(video.snapshots.at(-1)!.views,locale)} <i>{copy.views}</i>{growth&&<em>{` +${formatCompactNumber(growth.views,locale)}`}</em>}</b></span></a>})}</div>:<div className="ranking-empty">{emptyMessage||copy.noVideos}</div>;
  const renderChannels=()=>channelRows.length?channelRows.map((item,index)=><a className="ranking-row channel-ranking-row" key={item.channel.id} {...externalVideoProps(item.channel.url!)} title={interpolate(copy.channelTitle,{channel:item.channel.title})}><span className={cn('ranking-position',index<3&&'top')}>{String(index+1).padStart(2,'0')}</span><span className="channel-rank-avatar">{item.channel.thumbnail&&<img src={item.channel.thumbnail} alt={locale==='zh'?item.channel.title+' 的 YouTube 频道头像':item.channel.title+' YouTube channel avatar'} width={64} height={64} loading={index<3?'eager':'lazy'} fetchPriority={index<3?'high':'auto'} decoding="async"/>}</span><span className="ranking-title"><b>{item.channel.title}</b><small>{interpolate(copy.channelSamples,{count:item.videos.length,subs:formatCompactNumber(item.channel.subscribers,locale)})}</small></span><span className="ranking-stat"><b>{formatCompactNumber(item.views,locale)}</b><small>{copy.accumulatedViews}</small></span><span className="ranking-stat velocity"><b>{formatCompactNumber(Math.round(item.views/item.videos.length),locale)}</b><small>{copy.averageViews}</small></span><span className="channel-sample-count">{item.videos.length} {copy.samples}</span><span className="ranking-open">{locale==='zh'?'打开频道 ↗':'Open channel ↗'}</span></a>):<div className="ranking-empty">{emptyMessage||copy.noChannels}</div>;
