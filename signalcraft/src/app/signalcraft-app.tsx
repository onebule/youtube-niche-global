...
import { getBillingPortal, getBillingStatus, type BillingStatus } from '@/src/lib/billing';
...
function RankingBoard({longRows,shortRows,selectedFormat,filters,onDetail,savedVideoIds,savedChannelIds,onAddToStudio,locale,loadedCount,canLoadMore,loadingMore,onLoadMore,emptyMessage}:{longRows:Video[];shortRows:Video[];selectedFormat:'short'|'long';filters:ReturnType<typeof parseFilters>;onDetail:(v:Video)=>void;savedVideoIds:ReadonlySet<string>;savedChannelIds:ReadonlySet<string>;onAddToStudio:(video:Video,kind:'video'|'channel')=>void;locale:UiLocale;loadedCount:number;canLoadMore:boolean;loadingMore:boolean;onLoadMore:()=>void;emptyMessage:string|null}){
  ...
  return <section className="ranking-section top-100"><div className="ranking-section-head"><div><span className="eyebrow">{copy.corpus}</span><h2>{view==='videos'?rankingTitle:copy.channelTop}</h2><p>{view==='videos'?rankingBody:copy.channelTopBody}</p></div><b>{view==='videos'?videos.length:channelRows.length}<small>{locale==='zh'?(view==='videos'?' 条已加载':' 个频道'):(view==='videos'?' loaded':' channels')}</small></b></div>{view==='videos'&&<div className="ranking-lens" aria-label={copy.rankMethod}><span>{copy.lens}</span><button type="button" className={activeRankBy==='views'?'active':''} aria-pressed={activeRankBy==='views'} onClick={()=>setRankBy('views')}>{copy.absolute}</button><button type="button" className={activeRankBy==='relative'?'active':''} aria-pressed={activeRankBy==='relative'} onClick={()=>setRankBy('relative')}>{copy.relative}</button><button type="button" className={activeRankBy==='growth'?'active':''} aria-pressed={activeRankBy==='growth'} disabled={!hasComparableGrowth} title={!hasComparableGrowth?copy.growthUnavailable:undefined} onClick={()=>setRankBy('growth')}>{copy.growth}</button><small>{rankingBoundary}</small></div>}<div className={filters.display==='cards'?'ranking-card-wrap':'ranking-list'}>{view==='videos'?(filters.display==='cards'?renderCards():renderVideos()):renderChannels()}</div><div className="ranking-load-more"><span>{locale==='zh'?`当前已加载 ${loadedCount} 条已收录公开视频`:`${loadedCount} recorded public videos loaded`}</span>{canLoadMore?<button onClick={onLoadMore} disabled={loadingMore}>{loadingMore?(locale==='zh'?'正在加载下一页…':'Loading next page…'):(locale==='zh'?'加载更多':'Load more')}</button>:<small>{locale==='zh'?'当前筛选范围已无更多公开结果':'No more public results for this scope'}</small>}</div></section>
}
...
function Discovery({mode,state,setState,openDetail,locale}:{mode:'discover'|'rankings'|'radar'|'research';state:Persisted;setState:React.Dispatch<React.SetStateAction<Persisted>>;openDetail:(v:Video)=>void;locale:UiLocale}){
  ...
  if(mode==='rankings'){const selectedFormat=filters.format==='long'?'long':'short';const scopedVideos=[...rankingLongRows,...rankingShortRows].filter(video=>matchesRankingScope(video,filters)).filter(video=>video.format===selectedFormat);const scopedCount=filters.entity==='channels'?new Set(scopedVideos.map(video=>video.channelId)).size:scopedVideos.length;const comparableGrowthCount=scopedVideos.filter(video=>Boolean(getRecordedGrowth(video))).length;const copy=languageCopy[locale].ranking;const scopeLabel=rankingData?.dataScope?(locale==='zh'?'查看数据范围':'View data scope'):copy.loadingSamples;const resultLabel=filters.entity==='channels'?(locale==='zh'?<>当前筛选匹配 <b>{scopedCount}</b> 个频道（来自 {scopedVideos.length} 条已收录公开视频）</>:<>{scopedCount} channels match this filter ({scopedVideos.length} recorded public videos)</>):(locale==='zh'?<>当前筛选匹配 <b>{scopedCount}</b> 条已收录公开视频</>:<>{scopedCount} recorded public videos match this filter</>);return <main className="page rankings-page"><PageIntro label={copy.introLabel} title={copy.introTitle} body={copy.introBody}/><div className="rankings-layout"><RankingFilters filters={filters} setFilters={setFilters} locale={locale}/><div className="rankings-content"><div className="result-toolbar"><span>{resultLabel}</span><span>{scopeLabel}</span><button className="primary" onClick={()=>runRankingSearch(true)} disabled={loading}>{loading?copy.updating:copy.refresh}</button><button onClick={()=>navigator.clipboard?.writeText(location.href)}>{copy.copyLink}</button></div>{error&&<p className="api-error">{error}</p>}{rankingData?.dataScope&&<RankingDataScope scope={rankingData.dataScope} locale={locale} resultCount={scopedVideos.length} comparableGrowthCount={comparableGrowthCount}/>}<RankingBoard longRows={rankingLongRows} shortRows={rankingShortRows} selectedFormat={selectedFormat} filters={filters} onDetail={openDetail} savedVideoIds={savedVideoIds} savedChannelIds={savedChannelIds} onAddToStudio={addToStudio} locale={locale} loadedCount={rankingData?.loadedCount||0} canLoadMore={Boolean(rankingData?.nextPageToken)} loadingMore={loadingMore} onLoadMore={loadMoreRanking} emptyMessage={rankingData?.emptyMessage||null}/></div></div></main>;}
...
function BillingCenter(){
  ...
  return <main className="page"><PageIntro label="账单与订阅" title={active?'Pro 订阅已由服务端确认':'当前为 Free 账号'} body={active?'你的权益以 Waffo 已签名的订阅状态为准；取消中状态会保留至当前周期结束。':'升级后会在 Waffo 托管结算完成并由签名回调确认后生效。'}/><section className="billing-summary"><span className="eyebrow">ACCOUNT ENTITLEMENT</span><h2>{active?'Pro':'Free'} · {status?.status||'none'}</h2>{active&&<p>周期：{status?.billingCycle==='yearly'?'年付':'月付'} · 当前有效至 {status?.currentPeriodEnd?new Date(status.currentPeriodEnd).toLocaleString('zh-CN'):'待确认'}</p>}<div><button className="primary" onClick={()=>active?void openPortal():navigate('/pricing')}>{active?'管理订阅':'查看升级方案'}</button>{active&&<button onClick={()=>void refresh()}>刷新状态</button>}</div></section></main>;
}
