...
function RankingBoard({longRows,shortRows,selectedFormat,filters,onDetail,savedVideoIds,savedChannelIds,onAddToStudio,locale,loadedCount,canLoadMore,loadingMore,onLoadMore,emptyMessage}:{longRows:Video[];shortRows:Video[];selectedFormat:'all'|'short'|'long';filters:ReturnType<typeof parseFilters>;onDetail:(v:Video)=>void;savedVideoIds:ReadonlySet<string>;savedChannelIds:ReadonlySet<string>;onAddToStudio:(video:Video,kind:'video'|'channel')=>void;locale:UiLocale;loadedCount:number;canLoadMore:boolean;loadingMore:boolean;onLoadMore:()=>void;emptyMessage:string|null}){
  ...
  const renderVideos=()=>videos.length?videos.map((video,index)=>{const score=scoreFor(video),channel=channelFor(video),growth=getRecordedGrowth(video),snapshot=video.snapshots.at(-1)!,saved=savedVideoIds.has(video.id),sourceProps=externalVideoProps(video.sourceUrl!);return <div className="ranking-row ranking-video-row" key={video.id}><span className={cn('ranking-position',index<3&&'top')}>{String(index+1).padStart(2,'0')}</span><a className="ranking-thumb" {...sourceProps} title={locale==='zh'?'在新标签页打开此视频':'Open this video in a new tab'}>{video.thumbnail&&<img src={video.thumbnail} alt="" width={160} height={90} loading="lazy" decoding="async"/>}<em>{video.format==='short'?(locale==='zh'?'短':'Short'):(locale==='zh'?'长':'Long')}</em>{growth&&<small className="ranking-thumb-growth">+{formatCompactNumber(growth.views,locale)}</small>}</a><a className="ranking-title" {...sourceProps}><b title={video.title}>{video.title}</b><span className="ranking-channel">{channel.thumbnail&&<img src={channel.thumbnail} alt="" width={24} height={24} loading="lazy" decoding="async"/>}<span><strong>{channel.title}</strong><small>{localizedTopic(video.topic,locale)} · {formatCompactNumber(channel.subscribers,locale)} {locale==='zh'?'订阅':'subs'}</small></span></span></a><span className="ranking-stat ranking-stat-views"><b>{formatCompactNumber(snapshot.views,locale)}</b><small>{copy.views}</small></span><span className="ranking-stat ranking-stat-duration"><b>{formatVideoDuration(video.durationSeconds,locale)}</b><small>{formatPublishedAge(video.publishedAt,locale)}</small></span><span className="ranking-stat velocity"><b>{score.viewsPerSubscriber}×</b><small>{copy.viewsPerSubscriber}</small></span><span className="ranking-stat growth"><b>{growth?`+${formatCompactNumber(growth.views,locale)}`:'—'}</b><small>{growth?interpolate(copy.growthOverHours,{hours:Math.round(growth.hours)}):copy.growthUnavailable}</small></span><button type="button" className={cn('ranking-studio-action',saved&&'saved')} aria-label={saved?(locale==='zh'?`打开工作室中的视频：${video.title}`:`Open saved video in Studio: ${video.title}`):(locale==='zh'?`将视频加入工作室：${video.title}`:`Add video to Studio: ${video.title}`)} onClick={()=>onAddToStudio(video,'video')}>{saved?(locale==='zh'?'已加入 · 打开':'Added · Open'):(locale==='zh'?'加入工作室':'Add to Studio')}</button></div>}):<div className="ranking-empty">{emptyMessage||copy.noVideos}</div>;
  const renderCards=()=>videos.length?<div className="ranking-card-grid">{videos.map((video,index)=>{const channel=channelFor(video),growth=getRecordedGrowth(video),snapshot=video.snapshots.at(-1)!,score=scoreFor(video),saved=savedVideoIds.has(video.id),sourceProps=externalVideoProps(video.sourceUrl!);return <article className="ranking-video-card" key={video.id}><a className="ranking-card-image" {...sourceProps} title={locale==='zh'?'在新标签页打开此视频':'Open this video in a new tab'}>{video.thumbnail&&<img src={video.thumbnail} alt="" width={480} height={270} loading={index<3?'eager':'lazy'} fetchPriority={index<3?'high':'auto'} decoding="async"/>}<b>#{index+1}</b><em>{video.format==='short'?(locale==='zh'?'短':'Short'):(locale==='zh'?'长':'Long')}</em></a><span><a className="ranking-card-title" {...sourceProps}><strong>{video.title}</strong><span className="ranking-card-channel">{channel.thumbnail&&<img src={channel.thumbnail} alt="" width={24} height={24} loading="lazy" decoding="async"/>}<small>{channel.title} · {localizedTopic(video.topic,locale)}</small></span></a><div className="ranking-card-metrics"><b>{formatCompactNumber(snapshot.views,locale)} <i>{copy.views}</i></b><b>{formatVideoDuration(video.durationSeconds,locale)} <i>{formatPublishedAge(video.publishedAt,locale)}</i></b><b className="ranking-card-score">{score.opportunityScore}<i>{locale==='zh'?'机会分':'score'}</i></b></div>{growth&&<small className="ranking-card-growth">+{formatCompactNumber(growth.views,locale)} {locale==='zh'?'新增播放':'new views'}</small>}<button type="button" className={cn('ranking-card-studio',saved&&'saved')} onClick={()=>onAddToStudio(video,'video')}>{saved?(locale==='zh'?'已加入 · 打开':'Added · Open'):(locale==='zh'?'加入工作室':'Add to Studio')}</button></span></article>})}</div>:<div className="ranking-empty">{emptyMessage||copy.noVideos}</div>;
