...
function VideoCard({video,state,setState,onDetail}:{video:Video;state:Persisted;setState:React.Dispatch<React.SetStateAction<Persisted>>;onDetail:(v:Video)=>void}){
  ...
  return <article className="video-card">
    <div className={cn('thumb',portrait&&'thumb-portrait')} role="link" tabIndex={0} onClick={watch} onKeyDown={event=>event.key==='Enter'&&watch()} title="打开并观看此视频">
      {video.thumbnail&&<img className="video-card-image" src={video.thumbnail} alt={`${video.title} 视频缩略图`} width={480} height={270} loading="lazy" decoding="async" onError={event=>{event.currentTarget.style.display='none'}}/>}
