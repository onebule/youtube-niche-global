...
type RankingData = {short:Video[];long:Video[];nextPageToken:string|null;loadedCount:number;dataScope:PublicRankingScope|null};
  ...
  const loadMoreRanking=async()=>{
    ...
    try{
      ...
      setRankingData(current=>{
        ...
        return {short:merged.filter(video=>video.format==='short'),long:merged.filter(video=>video.format==='long'),nextPageToken:result.nextPageToken,loadedCount:merged.length,dataScope:result.dataScope||current.dataScope};
      });
