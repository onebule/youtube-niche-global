...
export function normalizeLongformResponse(payload: unknown): LongformResponse {
  ...
  const enrichedOpportunities = opportunities.map(opportunity => {
    ...
    const scriptDevelopment = buildScriptDevelopmentIntelligence({
      creativeDevelopment,
      capturedAt: nullableText(rawScope.latestCapturedAt),
      snapshotId: evidence.snapshotId || null,
      entryDecision: opportunity.entryDecision?.status || opportunityAssessment.decision.status,
    ...
    const storyboardIntelligence = buildStoryboardIntelligence({
      scriptDraft: scriptWriting.drafts[0] || null,
      scriptDevelopment,
      capturedAt: nullableText(rawScope.latestCapturedAt),
      snapshotId: evidence.snapshotId || null,
    });
    const storyboardForAssets = storyboardIntelligence.storyboards[0] || storyboardIntelligence.blockedStoryboards[0] || null;
    const visualAssetIntelligence = buildVisualAssetIntelligence({
