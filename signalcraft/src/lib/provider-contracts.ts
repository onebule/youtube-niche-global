...
function modelContract(item: typeof VIDEO_MODEL_DISCOVERY_FIXTURE.models[number]): ModelContract {
  ...
  return { providerId: item.provider || 'unknown', modelId: item.id, displayName: item.id === 'minimax-h3' ? 'MiniMax H3' : item.id === 'seedance-2-5' ? 'Seedance 2.5' : 'Seedance 2.0', actualRequestModelId: downstreamModel, operations: [{ operation: 'IMAGE_TO_VIDEO', state: 'VERIFIED', sourceReference: item.id === 'minimax-h3' ? minimaxReference : seedanceReference }, { operation: 'VIDEO_SUBMISSION', state: 'VERIFIED', sourceReference: apimartReference }, { operation: 'VIDEO_STATUS', state: 'VERIFIED', sourceReference: apimartReference }, { operation: 'VIDEO_OUTPUT', state: 'VERIFIED', sourceReference }], capabilities, inputConstraints: { referenceCount: 9, durationSeconds: duration, aspectRatios: ['9:16', '16:9', '1:1'], resolutions }, outputConstraints: { type: 'VIDEO', representation: 'PRIVATE_ASSET_ID_AFTER_SUPABASE_STORAGE', expiration: 'UNKNOWN' }, availability: item.enabled ? 'AVAILABLE' : 'DISABLED', verification: 'VERIFIED', executionReadiness: item.enabled ? 'READY_FOR_EXECUTION_INTEGRATION' : 'MODEL_DISABLED', provenance: { sourceType: 'REPOSITORY_CONFIG', sourceReferences: [sourceReference, apimartReference, item.id === 'minimax-h3' ? minimaxReference : seedanceReference], capturedAt: VIDEO_MODEL_DISCOVERY_FIXTURE.capturedAt, contractVersion: MODEL_CONTRACT_VERSION, notes: [item.reason, `下游请求模型：${downstreamModel}`] } };
}
