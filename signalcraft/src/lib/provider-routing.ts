...
import { applyVerifiedModelDiscovery, buildProviderVerificationReport } from './provider-contracts.ts';
import type { ProviderVerificationReport } from './provider-contracts.ts';
...
export type ProviderRoutingReport = { schemaVersion: 'provider-routing.v1'; algorithmVersion: typeof PROVIDER_ROUTING_ALGORITHM_VERSION; scope: 'LONG_FORM'; registry: ProviderRoutingRegistry; routes: ProviderRoutingDecision[]; gaps: string[]; verification?: ProviderVerificationReport; provenance: { source: 'P4_PHASE_3_CANONICAL_SPECIFICATION'; capturedAt: string | null; snapshotId: string | null; algorithmVersions: string[] } };
...
export const PROVIDER_ROUTING_REGISTRY: ProviderRoutingRegistry = Object.freeze({
  ...
  providers: [
    { providerId: 'minimax', displayName: 'MiniMax', generationDomains: ['IMAGE', 'VIDEO'], availability: 'CONFIGURED', verification: 'CONFIGURED', modelIds: ['minimax-h3'], adapterId: 'minimax-boundary', configurationVersion: 'video-gateway-model-contract-v1' },
    { providerId: 'seedance', displayName: 'Seedance', generationDomains: ['VIDEO'], availability: 'CONFIGURED', verification: 'CONFIGURED', modelIds: ['seedance-2', 'seedance-2-5'], adapterId: 'seedance-boundary', configurationVersion: 'video-gateway-model-contract-v1' },
  ],
  models: [
    { modelId: 'minimax-h3', providerId: 'minimax', displayName: 'MiniMax H3', availability: 'DISABLED', verification: 'CONFIGURED', capabilities: unknownCapabilities(), adapterId: ADAPTER_VERSION.minimax, configurationVersion: 'video-gateway-model-contract-v1', telemetry: { priceDataState: 'UNKNOWN', qualityDataState: 'UNKNOWN', speedDataState: 'UNKNOWN' } },
    { modelId: 'seedance-2', providerId: 'seedance', displayName: 'Seedance 2.0', availability: 'DISABLED', verification: 'CONFIGURED', capabilities: unknownCapabilities(), adapterId: ADAPTER_VERSION.seedance, configurationVersion: 'video-gateway-model-contract-v1', telemetry: { priceDataState: 'UNKNOWN', qualityDataState: 'UNKNOWN', speedDataState: 'UNKNOWN' } },
    { modelId: 'seedance-2-5', providerId: 'seedance', displayName: 'Seedance 2.5', availability: 'DISABLED', verification: 'CONFIGURED', capabilities: unknownCapabilities(), adapterId: ADAPTER_VERSION.seedance, configurationVersion: 'video-gateway-model-contract-v1', telemetry: { priceDataState: 'UNKNOWN', qualityDataState: 'UNKNOWN', speedDataState: 'UNKNOWN' } },
  ],
...
export function buildProviderRoutingReport(input: { specifications: VisualGenerationSpecification[]; blockedSpecifications?: VisualGenerationSpecification[]; registry?: ProviderRoutingRegistry; capturedAt?: string | null; snapshotId?: string | null }): ProviderRoutingReport {
  ...
  const registry = input.registry || PROVIDER_ROUTING_REGISTRY;
  return { schemaVersion: 'provider-routing.v1', algorithmVersion: PROVIDER_ROUTING_ALGORITHM_VERSION, scope: 'LONG_FORM', registry: { ...registry, models: applyVerifiedModelDiscovery(registry.models) }, routes, gaps: uniq(routes.flatMap(route => route.decisions.flatMap(decision => [...decision.blockers.map(blocker => blocker.message), ...decision.risks.map(risk => risk.message)]))), verification: buildProviderVerificationReport(input.capturedAt || null), provenance: { source: 'P4_PHASE_3_CANONICAL_SPECIFICATION', capturedAt: input.capturedAt || null, snapshotId: input.snapshotId || null, algorithmVersions: [PROVIDER_ROUTING_ALGORITHM_VERSION, PROVIDER_REGISTRY_VERSION, PROVIDER_CAPABILITY_VERSION] } };
}
