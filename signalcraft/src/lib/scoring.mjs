...
export function parseFilters(search) {
  ...
  return { q: p.get('q') || '', window: p.get('window') || '7d', language: p.get('language') || 'all', format: p.get('format') || 'all', maxSubs: p.get('maxSubs') || '100000', minScore: p.get('minScore') || '70', sort: p.get('sort') || 'score' };
}
