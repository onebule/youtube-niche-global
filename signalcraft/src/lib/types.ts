...
export interface Channel { id:string; title:string; handle:string; subscribers:number; language:string; region:string; medianViews:number; health:number; tags:string[]; owner:string; lastSync:string }
export interface VideoSnapshot { capturedAt:string; views:number; likes:number; comments:number; subscribers?:number }
export interface Video { id:string; channelId:string; title:string; topic:string; language:string; region:string; format:VideoFormat; publishedAt:string; durationSeconds:number; thumbnail:string; risk:'low'|'medium'|'high'; snapshots:VideoSnapshot[]; tags:string[]; }
