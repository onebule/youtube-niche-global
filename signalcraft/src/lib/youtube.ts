...
type ApiOpportunity = {
  ...
  ageDays:number; format:'short'|'long'; breakoutRatio?:number; viralLabel?:string;
};
...
export async function searchYouTubeSignals(input:{query:string;language:string;window:string}){
  const days = input.window==='24h'?30:input.window==='7d'?30:90;
  const params=new URLSearchParams({query:input.query,language:languageCode[input.language]||'en',region:'US',recentDays:String(days)});
  ...
  const videos:Video[]=source.map((item,index)=>{
    ...
    const publishedAt=new Date(Date.now()-Math.max(item.ageDays,1)*86400000).toISOString();
    ...
    return {id:`yt-${index}-${item.title.slice(0,18)}`,channelId,title:item.title,topic:input.query,language:input.language==='all'?'英语':input.language,region:'美国',format:item.format,durationSeconds:item.format==='short'?55:480,thumbnail:item.thumbnail||'',publishedAt,risk:'medium',tags:['YouTube 公开数据','单次快照',item.viralLabel||''],snapshots:[{capturedAt:new Date().toISOString(),views:item.views,likes:0,comments:0,subscribers:item.subscribers}]};
  });
